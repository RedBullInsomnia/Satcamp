...
namespace SatelliteServer
{
    public partial class Window : Form
    {
        ...
        public Window()
        {
            ...
            _updateTimer = new System.Timers.Timer(25); // The system will update every 25 ms
        ...
        void _updateTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            this.Invoke(new Action(() =>
            {
                if (_um6Driver != null)
                {
                    tbPitch.Text = _um6Driver.Angles[1].ToString("F1");
                    tbYaw.Text = _um6Driver.Angles[2].ToString("F1");
                    tbRoll.Text = _um6Driver.Angles[0].ToString("F1"); //Precision of 1 decimal
                ...
                if (stabilizeCb.Checked)
                {
                    ...
                    double dPitch = _um6Driver.Angles[1] - Stab_angles[1]; //- _stabPitchServo;
                    //Console.WriteLine("Dpitch = " + dPitch);
                    int tmp_pitchtrackbarvalue = _stabPitchServo + (int)(dPitch * pid_paramp * PitchAngleCoefficient);
                    if (tmp_pitchtrackbarvalue < pitchTrackBar.Minimum)
                        pitchTrackBar.Value = pitchTrackBar.Minimum; // + (int)(dPitch * PitchAngleCoefficient);
                    else if(tmp_pitchtrackbarvalue > pitchTrackBar.Maximum)
                        pitchTrackBar.Value = pitchTrackBar.Maximum; // + (int)(dPitch * PitchAngleCoefficient);
                    else
                        pitchTrackBar.Value = tmp_pitchtrackbarvalue;

                    // Calculate the error on the yaw axis
                    double dYaw = _um6Driver.Angles[2] - Stab_angles[2]; // -_stabYawServo;
                    //Console.WriteLine("Dyaw = " + dYaw);
                    int tmp_yawtrackbarvalue = _stabYawServo + (int)(dYaw * pid_paramp * PitchAngleCoefficient);
                    if (tmp_yawtrackbarvalue < yawTrackBar.Minimum)
                        yawTrackBar.Value = yawTrackBar.Minimum; // + (int)(dYaw * PitchAngleCoefficient)
                    else if (tmp_yawtrackbarvalue > yawTrackBar.Maximum)
                        yawTrackBar.Value = yawTrackBar.Maximum;
                    else
                        yawTrackBar.Value = tmp_yawtrackbarvalue;
                }
            }));
        ...
        private void stabilizeCb_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                Stab_angles[0] = _um6Driver.Angles[0];// *Math.PI / 180.0; //Roll
                Stab_angles[1] = _um6Driver.Angles[1];// *Math.PI / 180.0; //Pitch
                Stab_angles[2] = _um6Driver.Angles[2];// *Math.PI / 180.0; //Yaw
        ...
        private void groupBoxServos_Enter(object sender, EventArgs e)
        {
